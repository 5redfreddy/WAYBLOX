...
function checkAndConnect(ipText, playerName)
	local success, err = pcall(function()	  
		...
		client.Ticket = "david" 	 
		playerConnectSucces, player = pcall(function() return client:PlayerConnect(0, tostring(ipText), 53640, 0, threadSleepTime) end)  
...
function createIPbox()
	...
	box_name.FocusLost:connect(function(enter)
		if enter then
			nameToSet = box_ip.Text
		end
	end)
end
